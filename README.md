...
1. 요약: **지역변수** 때문에 메서드 추출을 적용할 수 없는 긴 메서드가 있을 땐 객체로 전환시켜 지역변수를 객체의 필드로 만들자. 그리고 그 메서드를 여러 개로 쪼개면 된다.

2. 동기: **지역변수**가 많으면 메서드를 쪼개기 어려워진다. 많은 지역변수를 가진 메서드를 쪼개기 위해 이 기법을 적용하면 메서드 추출하기가 편해진다.

3. 절차

- 메서드와 이름이 같은 새 클래스를 만든다.
- 클래스에 **final** 필드로 원본 메서드의 객체를 선언한다. 원래 메서드의 임시변수와 매개변수를 클래스의 필드로 선언한다.
- 새 클래스에 원본 객체와 매개변수를 받는 생성자를 선언한다.
- 새 클래스에 원본 메서드의 내용을 복사해 넣고, 매개변수는 앞서 선언한 필드로 대체한다.
- 컴파일을 실시한다.
- 원본 메서드에는 새 객체 **생성자를 호출**하고 복사된 **메서드를 호출**하는 코드만 남긴다.

### Substitute Algorithm

1. 요약: 알고리즘을 더 쉬운 것으로 교체가 가능할 땐 해당 메서드를 새 알고리즘을 바꾼다.

2. 동기: 다른 코드와 마찬가지로 알고리즘도 더 간단한 방법이 있다면 복잡한 것을 간단한 것으로 교체해야 한다. 라이브러리의 일부 코드가 자신의 코드와 중복될 때도 마찬가지의 상황이다. 이 작업을 진행하기 앞서서 메서드를 최대한 잘게 쪼개야 한다.

3. 절차

- 교체할 간결한 알고리즘을 준비하여 컴파일을 실시한다.
- 새 알고리즘을 여러 번 테스트를 해보고 결과가 같은지 검사한다.
- 결과가 다르면 기존의 알고리즘으로 테스트와 디버깅을 실시해 비교한다.
- 각각의 알고리즘에 대해 테스트 케이스를 실행하고 비교하면 어떤 테스트 케이스가 문제가 되는지 확인할 수 있다.

## 객체 간의 기능 이동

객체를 설계할 때 가장 중요한 일 중 하나는 **기능을 어디에 넣을지 판단**하는 것이다. 

### Move Method

1. 요약: 메서드가 자기가 속한 클래스보다 다른 클래스의 기능을 더 많이 이용할 때는 그 클래스 안에서 새로 작성하고 기존의 메서드는 대리 메서드로 전환하거나 삭제하자.

2. 동기: 클래스에 기능이 너무 많거나 다른 클래스와 과하게 연동되어 의존성이 지나치면 메서드를 옮기는 것이 좋다. 옮길 만한 메서드가 발견되면 **그 메서드와 참조되거나 그 메서드가 참조하는 메소드**를 잘 살펴보고 더 많이 참조하는 메서드가 있는 객체를 기준으로 진행할지 판단한다.

3. 절차

- 옮기려는 메서드에 있는 모든 기능을 검사하고 옮길 필요가 있는지 판단한다. 연관되는 기능 중 옮기는 것이 더 낫다고 판단되는 것이 있다면 하나씩 옮기는 것보다는 한꺼번에 옮기는 것이 편할 수도 있기 때문이다.
- 상위와 하위클래스에서 그 메서드에 대한 상속관계가 있는지 확인한다. 있다면 재정의된 메서드도 옮길 수 있을 때만 이 방법이 적용 가능할지도 모른다.
- 메서드를 대상 클래스에 선언한다.
- 옮겨진 메서드를 잘 돌아가게끔 수정한다.
- 대상 클래스를 컴파일한다.
- 옮겨진 메서드가 기존의 클래스의 기능을 필요로 할 때는 옮겨진 메서드가 기존 클래스를 참조할 방법을 정하거나, 기존 메서드가 매개변수를 통해 필요한 기능을 옮겨진 메서드로 전달해야 한다. 이 경우에는 기존의 메서드를 간단한 위임 코드로 바꿀 수 있다.
- 컴파일과 테스트를 진행한다.
- 원본 메서드를 삭제할 지 결정하고, 삭제할 때는 기존의 메서드를 참조하는 코드를 새로 만든 클래스의 메서드를 참조하도록 수정한다.
- 컴파일과 테스트를 진행한다.

### Move Field

1. 요약: 필드가 자신이 속한 클래스보다 다른 클래스에서 더 많이 사용될 때는 대상 클래스 안에 필드를 옮기고 참조 부분을 수정한다

2. 동기: 메서드를 옮기는 방법을 사용할 수도 있지만 메서드의위치가 적절하다고 판단되면 필드를 옮긴다. 클래스를 추출할 때느 필드를 옮기는 기법이 수반된다. 이럴 땐 필드가 우선이고 메서드는 다음이다.

3. 절차

- 필드가 public 이면 [Encapsulate Field](#Encapsulate-Field)기법을 실시하자.
- 필드에 자주 접근하는 메서드를 옮기게 될 가능성이 높거나 필드에 많은 메서드가 접근할 때는 [Self Encapsulate Field](#Self-Encapsulate-Field)를 실시하자.
- 컴파일과 테스트를 실시한다.
- 대상 클래스에 get/set 메서드와 함께 필드를 작성하자.
- 대상 클래스를 컴파일한다.
- 원본 객체에서 대상 객체를 참조할 방법을 정하자. 간단한 메서드를 만들거나 그것이 힘들다면 대상 클래스를 저장할 수 있는 새 필드를 원본 클래스에 만들어야 한다. 이렇게 만들어진 필드는 리팩토링 후에 삭제할 수 있다.
- 원본 클래스에서 필드를 삭제하자.
- 원래의 필드를 참조하는 모든 부분을 get/set 메서드를 이용하여 수정한다. 필드가 private이 아니라면 하위 클래스에서 필드를 참조하는 부분도 찾아내야 한다.
- 컴파일과 테스트를 진행한다.

### Extract Class

1. 요약: 두 클래스가 처리해야 할 기능이 하나의 클래스에 들어 있을 때, 새 클래스를 만들고 기존 클래스의 관련 필드와 메서드를 새 클래스로 옮기자.

2. 동기: 클래스는 확실하게 추상화 되어야 하지만, 개발이 진행될 수록 기능이 추가되어 복잡해진다. 결국 클래스의 어느 부분을 떼어내야 한다. 데이터의 일부분과 메서드의 일부분이 **한 덩어리**이거나, 주로 함께 변화하여 **의존적인 데이터**의 일부분도 떼어내기 좋다. 이것을 판단하는 좋은 방법은 데이터나 메서드를 떼어내거나 새로 추가할 때 합리적이지 않은지 자문해보는 것이다. 

3. 절차

- 클래스의 기능 분리 방법을 정한다.
- 분리한 기능을 넣는 새 클래스를 작성한다.
-> 분리된 클래스와 원본 클래스의 이름을 어울리게 바꾼다.
- 원본 클래스에서 새 클래스로의 링크를 만들자.
- Move Field를 적용한다.
- 하나씩 옮길 때마다 컴파일과 테스트를 실시한다.
- Move Method를 실시한다. 하급(피호출) 메서드부터 상급(호출) 메서드에 적용하자.
- 마찬가지로 컴파일과 테스트를 실시한다.
- 각 클래스를 다시 검사해서 인터페이스를 줄이고 양방향 링크 대신 단방향 링크로 바꿀 수 있는지 확인한다.
- 새로 만든 클래스의 접근성에 대해 결정하자. 여러 곳에서 접근할 수 있게 할 경우, 새 클래스를 참조(reference) 객체나 변경불가 값(value) 객체로서 공개할지 여부를 결정하자.
- 접근성에 따라 새 클래스의 값을 변경하는 주체를 정하여 그 클래스에서만 수정이 가능하게 만들어야 한다.
